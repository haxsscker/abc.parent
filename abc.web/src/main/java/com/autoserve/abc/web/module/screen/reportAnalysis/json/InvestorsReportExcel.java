...
public class InvestorsReportExcel {
    ...
    public void execute(Context context, ParameterParser params,Navigator nav) {
		...
		if(startInvestDate != null){
			investorsReportDO.setStartInvestDate(startInvestDate);
		}else if(endInvestDate != null){
			investorsReportDO.setEndInvestDate(endInvestDate);
		}
    	PageResult<InvestorsReportDO> result = investQueryService.queryInvestorsReport(investorsReportDO, new PageCondition(0, investQueryService.queryCountInvestorsReport(investorsReportDO)));
    	
		List<String> fieldName=Arrays.asList(new String[]{"项目代码","投资人用户名","姓名","身份证号","手机号","邮箱","推荐人","投资时间","投资金额","应收利息","已收利息","已收本金","借款到期日","是否转让","转让时间","是否结清"});

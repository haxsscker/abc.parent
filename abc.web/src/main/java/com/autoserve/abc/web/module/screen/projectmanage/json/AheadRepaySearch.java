 ...
 * 
 */
public class AheadRepaySearch {
	private final static Logger logger = LoggerFactory.getLogger(AheadRepaySearch.class);
	@Resource
	...
	public JsonBaseVO execute(@Param("aheadRepayId") int aheadRepayId,Context context) {
		JsonBaseVO result = new JsonBaseVO();
		AheadRepay aheadRepay  = new AheadRepay();
		aheadRepay.setId(aheadRepayId);
		aheadRepay = aheadRepayDao.findOne(aheadRepay);
		if(aheadRepay.getState()!=AheadRepayState.AUDIT_WAITING){
			result.setSuccess(false);
			result.setMessage("该记录已"+aheadRepay.getState().desc);
			return result;
		}
		// 查询还款计划
		List<PaymentPlanDO> paymentList = paymentPlanDao.findByLoanId(aheadRepay.getLoanId());
		PaymentPlanDO nextPayment = this.getNextPayment(paymentList);
		String seqNo = nextPayment!=null?nextPayment.getPpInnerSeqNo():"";
		if(null == nextPayment){//已还清
			aheadRepay.setState(AheadRepayState.AUDIT_PASS);
			aheadRepayDao.update(aheadRepay);
			result.setMessage("项目已还清！");
			return result;
		}
		...
		paramMap.put("aheadRepayId", String.valueOf(aheadRepayId));
	...
	private PaymentPlanDO getNextPayment(List<PaymentPlanDO> paymentList){
		PaymentPlanDO nextPayment = null;// 下期还款计划
		for (PaymentPlanDO p : paymentList) {
			if (p.getPpPayState() == PayState.PAYING.state) {
				nextPayment = p;
				break;
			}
		}
	...
	private JsonBaseVO doMoneyTransfer(Map<String, String> params){
		...
		String aheadRepayId = params.get("aheadRepayId");
		AheadRepayState aheadRepayState = AheadRepayState.AUDIT_PASS;
		...
		try {
			...
			if (CollectionUtils.isEmpty(dealRecords)) {
				...
				if("000000".equals(RespCode)){
					...
					aheadRepayState = AheadRepayState.WAIT_AUDIT;//重新审核
					result.setSuccess(false);
				...
				//更新提前还款审核记录
				AheadRepay aheadRepay  = new AheadRepay();
				aheadRepay.setId(Integer.valueOf(aheadRepayId));
				aheadRepay = aheadRepayDao.findOne(aheadRepay);
				aheadRepay.setState(aheadRepayState);
				aheadRepayDao.update(aheadRepay);

...
public class BasicInformationEdit {
	...
	public JsonBaseVO execute(Context context, Navigator nav,
		...
		if (user != null) {
			PlainResult<UserDO> userDO = this.userService.findById(user
					.getUserId());
		...
		if (userEmail != null) {
			...
			// imageCaptchaService.validateResponseForID(request.getSession().getId(),
			// emailNum);
		...
		if (userPhone != null) {
			...
			if (null != Verification
					&& Md5Encrypt.md5(Verification).equals(validCode)) {
		...
		// if(userRealName!=null && !"".equals(userRealName) && userDocNo!=null
		// && !"".equals(userDocNo) && userPhone!=null &&
		// !"".equals(userPhone)){

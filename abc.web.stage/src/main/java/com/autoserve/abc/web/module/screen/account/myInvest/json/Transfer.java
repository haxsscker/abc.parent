...
public class Transfer {
	...
	private JsonBaseVO launchTransfer(int userId, int originId, double launchMoney, int investId, String flag) {
		...
		if (periodTypeResult.getData().getConfValue().equals("1")) {
			...
			flagDay = fullTransferedtime.plusDays(Integer.valueOf(periodResult.getData().getConfValue()))
			...
			flagDay = fullTransferedtime.plusMonths(Integer.valueOf(periodResult.getData().getConfValue()))
					.compareTo(DateTime.now());
			if (flagDay > 0) {
				result.setSuccess(false);
		...
		if (limitResult.getData().getConfValue() != null && !"-1".equals(limitResult.getData().getConfValue())) {
			...
			PageResult<TransferLoan> transferResult = transferLoanService.queryListByParam(transferLoan,
					new PageCondition(1, 65535));
		...
		pojo.setTransferPeriod(paymentPlanDOList.size() - collectPeriod); // 转让期数
																			// =总期数-已还期数

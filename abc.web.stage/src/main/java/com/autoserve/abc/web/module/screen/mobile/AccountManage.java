...
import com.autoserve.abc.web.module.screen.mobileIos.MobileHelper;
	...
	public JsonMobileVO execute(Context context, ParameterParser params)
		...
		try {
			...
			if ("1".equals(catalog)) {
				...
				if (null != account && StringUtils.isNotEmpty(account.getAccountNo())) {
					accountBalance = this.doubleDryService.queryBalance(account.getAccountNo(), "1");
					resultMap.put("accountUserAccount", account.getAccountUserAccount());
					resultMap.put("AccountName", account.getAccountUserAccountName());
					resultMap.put("AccountBk", account.getAccountUserAccountBk());
					if(userDO.getUserType()==2){
						String accountUserAccount =account.getAccountUserAccount();
						String mark=account.getAccountMark();
				...
				if (null != account && StringUtils.isNotEmpty(account.getData().getAccountNo())) {
...
//				result = unBindBankCard(params);
	...
	private JsonMobileVO modifyPlatPsw(ParameterParser params) {
        ...
        String type = params.getString("type");//1个人找回密码 ，2个人修改密码，3企业找回密码，4企业修改密码
    	UserIdentity userIdentity = new UserIdentity();
...
//	    	}
		...
		if (user.getUserType().getType() == 1) {
			resultMap.put("userPhone", MobileHelper.nullToEmpty(user.getUserPhone()));
		} else {
			resultMap.put("userPhone", MobileHelper.nullToEmpty(companyCustomerDao.findByUserId(userId).getCcContactPhone()));
		}
		UserDO userDO = new UserDO();

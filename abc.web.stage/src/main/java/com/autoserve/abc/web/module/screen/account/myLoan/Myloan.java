...
public class Myloan {
	...
	public void execute(Context context, ParameterParser params) {
		...
		if(LoanStateType==15)
		{
			...
			//查询还款计划的提前还款状态 add by 夏同同 20160506
			AheadRepay t = null;
			if(myBorrowReceivedList!=null && myBorrowReceivedList.size()>0){
				for (MyBorrowReceived mr : myBorrowReceivedList){
					t = new AheadRepay();
					t.setLoanId(Integer.parseInt(mr.getLoanId()));
					t = aheadRepayDao.findOne(t);
					if(t!=null){
						mr.setAheadState(t.getState().name());
					}
				}
			}

    (function( $ ) {
 	...
 	var drawCake = function(canvas,cakeImage){
		var myctx = canvas.getContext("2d");
		myctx.drawImage(cakeImage, 0, 0);
    ...
    $.fn.circliful = function(options) {
        ...
        var settings = $.extend({
         ...
         return this.each(function() {
				...
				if($(this).data('maincolor') != undefined) {
                ...
                if(settings.percent!== undefined || $(this).data('text') != undefined) {
                    text = $(this).data('text') || settings.percent + '%';
					...
					if($(this).data('icon') != undefined) {
					 ...
					 if($(this).data('type') != undefined) {
						...
						if(type == 'half') {
							...
							$(this).find('.circle-text-half').css({'line-height': (dimension / 1.45) + 'px', 'font-size' : size + 'px' , 'color': maincolor});
						} else {
							$(this).append('<span class="circle-text">' + icon + text + '</span>');
							$(this).find('.circle-text').css({'line-height': dimension + 'px', 'font-size' : size + 'px' ,'color': maincolor});
						}
					} else {
						/*$(this).append($('<span>', {
				...
				if($(this).data('info') != undefined) {
					...
					if($(this).data('type') != undefined) {
						...
						if(type == 'half') { 
							...
							$(this).find('.circle-info-half').css({'line-height': (dimension * 0.9) + 'px', 'color': maincolor });
						} else {
							$(this).append('<span class="circle-info">' + info + '</span>');
							$(this).find('.circle-info').css({'line-height': (dimension * 1.25) + 'px', 'color': maincolor  });
						}
					} else {
						$(this).append('<span class="circle-info">' + info + '</span>');
						$(this).find('.circle-info').css({'line-height': (dimension * 1.25) + 'px', 'font-size' : size/2 + 'px','color': maincolor  });
					}
                }

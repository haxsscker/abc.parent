package com.autoserve.abc.service.message.tsign;
...
import com.autoserve.abc.service.message.tsign.util.FileHelper;
...
public class SignManager {
	...
	private String signProjectSecret;
	...
	private void initProject()
	{
		...
		// 项目ID(应用ID)
		proCfg.setProjectId(signProjectId);// 1111563517
		// 项目Secret(应用Secret)
		proCfg.setProjectSecret(signProjectSecret); // 95439b0863c241c63a861b87d1e647b7
		// 开放平台地址
		proCfg.setItsmApiUrl(signApiUrl);
	 ...
	 * 关键字定位签署的PosBean
	 */
	public PosBean setKeyPosBean(String key,int x, int y,int width) {
		PosBean posBean = new PosBean();
		// 定位类型，0-坐标定位，1-关键字定位，默认0，若选择关键字定位，签署类型(signType)必须指定为关键字签署才会生效。
...
//		signPDFFileBean.setFileName("pdf文件名");
	 ...
	 * SignType.Single);
	 */
	public FileDigestSignResult platformSignByStreamm(String srcPdfFile, String page,int x,int y) {
		// 设置文件流签署的PDF文档信息
		...
		if (0 != fileDigestSignResult.getErrCode()) {
			System.out.println("平台自身PDF摘要签署（文件流）失败，errCode=" + fileDigestSignResult.getErrCode() + " msg="
	 ...
	 * SignType.Single);
	 */
	public FileDigestSignResult platformSignByFile(String srcPdfFile,String signedPdfFile, String page,int x,int y) {
		// 设置文件签署的PDF文档信息
		...
		FileDigestSignResult fileDigestSignResult = selfSignService.localSignPdf(signPDFFileBean, posBean, sealId,
				signType);
		if (0 != fileDigestSignResult.getErrCode()) {
			System.out.println("平台自身PDF摘要签署（文件）失败，errCode=" + fileDigestSignResult.getErrCode() + " msg="
					+ fileDigestSignResult.getMsg());
		} else {
	...
	public FileDigestSignResult userPersonSignByStream(byte[] pdfFileStream, String accountId,
			String sealData, String key,int x, int y,int width) {

		// 设置文件流签署的PDF文档信息
		SignPDFStreamBean signPDFStreamBean = setSignPDFStreamBean(pdfFileStream);
		// 设置坐标定位签署的PosBean，坐标定位方式支持单页签章、多页签章和骑缝章，但对关键字签章指定页码无效；
		PosBean posBean = setKeyPosBean(key,x,y,width);
		// 设置签署类型为 关键字签章
		...
		FileDigestSignResult fileDigestSignResult = userSignService.localSignPDF(accountId, sealData, signPDFStreamBean,
				posBean, signType);
		if (0 != fileDigestSignResult.getErrCode()) {
			System.out.println("平台个人客户的PDF摘要签署失败，errCode=" + fileDigestSignResult.getErrCode() + " msg="
					+ fileDigestSignResult.getMsg());
		} else {
	 ...
	 * signPDFStreamBean, posBean, SignType.Single);
	 */
	public FileDigestSignResult userPersonSignByFile(String srcPdf,String signedPdf,String accountId,String userName,
			String sealData) {
		
		// 设置文件流签署的PDF文档信息
		SignPDFFileBean signPDFFileBean = setSignPDFFileBean(srcPdf,signedPdf);
		// 设置坐标定位签署的PosBean，坐标定位方式支持单页签章、多页签章和骑缝章，但对关键字签章指定页码无效；
		PosBean posBean = setKeyPosBean(userName,0,0,159);
		// 设置签署类型为 关键字签章
		SignType signType = SignType.Key;
	...
	public FileDigestSignResult userPersonSignByFileXY(String srcPdf,String signedPdf,String accountId,String userName,
			String sealData,int x,int y,int width) {
		
		// 设置文件流签署的PDF文档信息
		SignPDFFileBean signPDFFileBean = setSignPDFFileBean(srcPdf,signedPdf);
		// 设置坐标定位签署的PosBean，坐标定位方式支持单页签章、多页签章和骑缝章，但对关键字签章指定页码无效；
		PosBean posBean = setKeyPosBean(userName,x,y,width);
		// 设置签署类型为 关键字签章
		SignType signType = SignType.Key;
	...
	/**
	 * 5.1平台自身PDF摘要签署（印章标识）
	 * FileDigestSignResult localSignPDF(SignPDFFileBean file, PosBean signPos, int sealId, SignType signType)
	 * sealId：签署印章的标识，为0表示用默认印章签署（需在E签宝上传合同印章）
	 * @param srcPdf
	 * @param signedPdf
	 * @param key
	 * @return
	 */
	public FileDigestSignResult platformSignByFile(String srcPdf, String signedPdf, String key) {
		
		// 设置文件流签署的PDF文档信息
		SignPDFFileBean signPDFFileBean = setSignPDFFileBean(srcPdf,signedPdf);
		// 设置坐标定位签署的PosBean，坐标定位方式支持单页签章、多页签章和骑缝章，但对关键字签章指定页码无效；
		PosBean posBean = setKeyPosBean(key,350,100,159);
		// 设置签署类型为 关键字签章
		SignType signType = SignType.Key;

		SelfSignService selfSignService = SelfSignServiceFactory.instance();
		FileDigestSignResult fileDigestSignResult = selfSignService.localSignPdf(signPDFFileBean,posBean, 0, signType);
		if (0 != fileDigestSignResult.getErrCode()) {
			System.out.println("平台PDF摘要签署失败，errCode=" + fileDigestSignResult.getErrCode() + " msg="
					+ fileDigestSignResult.getMsg());
		} else {
			System.out.println("平台PDF摘要签署成功！SignServiceId = " + fileDigestSignResult.getSignServiceId());
		}
		return fileDigestSignResult;
	}
	public FileDigestSignResult platformSignByFileXY(String srcPdf, String signedPdf, String key,int x,int y,int width) {
	...
	public FileDigestSignResult userOrganizeSignByStream(byte[] pdfFileStream, String accountId,
			String sealData, String page,int x,int y) {
		...
		FileDigestSignResult fileDigestSignResult = userSignService.localSignPDF(accountId, sealData, signPDFStreamBean,
				posBean, signType);
		if (0 != fileDigestSignResult.getErrCode()) {
			System.out.println("平台企业客户的PDF摘要签署失败，errCode=" + fileDigestSignResult.getErrCode() + " msg="
					+ fileDigestSignResult.getMsg());
		} else {
...
//		personBean.setPersonArea(0);
		...
		if (0 != addAccountResult.getErrCode()) {
			System.out.println("创建个人账户失败，errCode=" + addAccountResult.getErrCode() + " msg=" + addAccountResult.getMsg());
		} else {
	 ...
	 */
	public String addOrganizeAccount(OrganizeBean organizeBean) {
...
//		organizeBean.setAgentIdNo("220301198705170035");
		...
		System.out.println("----开始创建企业账户..." + organizeBean.getRegType() + "," + organizeBean.getOrganCode());
		AccountService accountService = AccountServiceFactory.instance();
		AddAccountResult addAccountResult = accountService.addAccount(organizeBean);

		if (0 != addAccountResult.getErrCode()) {
			System.out.println("创建企业账户失败，errCode=" + addAccountResult.getErrCode() + " msg=" + addAccountResult.getMsg());
		} else {
	...
	public void deleteAccount(String accountId){
		...
		if (0 != result.getErrCode()) {
			System.out.println("注销个人账户失败，errCode=" + result.getErrCode() + " msg=" + result.getMsg());
		} else {
	...
	public void updatePersonAccount(String accountId,String mobile){
		...
		if (0 != result.getErrCode()) {
			System.out.println("更新个人账户失败，errCode=" + result.getErrCode() + " msg=" + result.getMsg());
		} else {
	...
	public void updateOrganizeAccount(String accountId,String mobile){
		...
		if (0 != result.getErrCode()) {
			System.out.println("更新企业账户失败，errCode=" + result.getErrCode() + " msg=" + result.getMsg());
		} else {
	 ...
	 */
	public String addPersonTemplateSeal(String accountId) {
		String sealData = "";
		// 印章模板类型：矩形印章
		...
		if (0 != addSealResult.getErrCode()) {
			System.out.println("创建个人模板印章失败，errCode=" + addSealResult.getErrCode() + " msg=" + addSealResult.getMsg());
		} else {
			System.out.println("创建个人模板印章成功！SealData = " + addSealResult.getSealData());
			sealData = addSealResult.getSealData();
		}
		return sealData;
	}
	 ...
	 */
	public String addOrganizeTemplateSeal(String accountId) {
		/*
		...
		AddSealResult addSealResult = sealService.addTemplateSeal(accountId, organizeTemplateType, sealColor, hText,
				qText);
		if (0 != addSealResult.getErrCode()) {
			System.out.println("创建企业模板印章失败，errCode=" + addSealResult.getErrCode() + " msg=" + addSealResult.getMsg());
		} else {
	...
	public static boolean saveSignedByStream(byte[] signedStream,String signedFolder,String signedFileName) {
		...
		if (0 != Integer.parseInt(fileResult.get("errCode"))) {
			System.out.println("保存签署后文件失败，errCode=" + fileResult.get("errCode") + " msg=" + fileResult.get("msg"));
		} else {
			isSuccess = true;
	...
	public String getSignProjectSecret() {
		return signProjectSecret;
	}

	public void setSignProjectSecret(String signProjectSecret) {
		this.signProjectSecret = signProjectSecret;
	}

	public String getSignApiUrl() {
		return signApiUrl;
	}

	public void setSignApiUrl(String signApiUrl) {
